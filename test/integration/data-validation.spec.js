...
var { ProtocolError } = require('../../src/common/errors.js');
var protocol = require('../../src/background/protocol.js');
var { Parser } = require('../../src/background/parser.js');


describe('---------- request and response validation\
	(integration tests) ----------', () => {

	var parser = new Parser({ protocol, Ajv, sanitizeHtml });
	...
	describe('marker response validation', () => {
		...
		describe('protocol violation (misc)', () => {
			...
			it('should throw if response is not an object', () => {
				resp = null;
				expect(() => parse(resp)).toThrowError(ProtocolError);
		...
		describe('protocol fulfilment (misc)', () => {
			...
			it('should pass if response is well-formed (1)', () => {
				resp = {};
				expect(() => parse(resp)).not.toThrowError(ProtocolError);
			});
